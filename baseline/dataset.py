...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, args, tokenizer, split_type, labels=True, labels_file=None):
        ...
        ## dataset_walker is the class with logs and labels (with knoweldge addded)
        self.dialogs = self._prepare_conversations()
        self.knowledge_reader = KnowledgeReader(self.dataroot, args.knowledge_file)
        ##knowledge_reader is the helper class which helps in fetaching data from knowledge data based on the labels
        self.snippets = self._prepare_knowledge()
        ## snippets has the knoweldge tokenized with the key domain_entity_id_doc_id
    ...
    def _prepare_conversations(self):
        ...
        for i, (log, label) in enumerate(tqdm(self.dataset_walker, disable=False, desc='tokenizing...')):
            ...
            if label is not None:
                if "response" in label: ## this is to get the GT response
    ...
    def _create_examples(self):
        ...
        for dialog in tqdm(self.dialogs, disable=False, desc='creating examples'):
            ...
            if not target and self.args.task != "detection": ## if target is false then we don't care about it unless it's the detection task
            ...
            self.examples.append({
                ...
                "knowledge": used_knowledge, ## this is tokenized knowledge
...
class KnowledgeTurnDetectionDataset(BaseDataset):
    ...
    def build_input_from_segments(self, history):
        ...
        ## to make the question(last sentence) have a different token_type_ids, we make it 1. and keep the context as 0. we can
        ## have as many types of token_type_ids as we want
...
class KnowledgeSelectionDataset(BaseDataset):
    ...
    def build_input_from_segments(self, knowledge, history):
        ...
        sequence_with_speaker = sequence[-1]

        print("_________ sequence with speaker ___________")
        print(sequence_with_speaker)

        sequence0 = [self.cls] + sequence_with_speaker + [self.sep] ##utterance with specail tags/token
        sequence1 = knowledge + [self.sep] ##add knoweldge to it. 
