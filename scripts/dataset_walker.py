...
class DatasetWalker(object):
    ...
    def __iter__(self):
        if self.labels is not None:
            for log, label in zip(self.logs, self.labels): ##iterate through each dialogue
                if self._incl_knowledge is True and label['target'] is True: ##if need knoweldge
                    for idx, snippet in enumerate(label['knowledge']): ##go through all the knowledge in the each label file
                        ...
                        if doc_type == 'review':
                            ...
                            sent = self._knowledge.get_review_sent(domain, entity_id, doc_id, sent_id) ##fetch the correponding sentence from the knoweldge.json
                            label['knowledge'][idx]['sent'] = sent ## the sentence is added to the label doctionary itself
                            
                        elif doc_type == 'faq': ## similar to review sentence
